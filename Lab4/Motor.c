...
void MotorForward(volatile uint16_t leftDuty, volatile uint16_t rightDuty ){
...
// Output: none
// Assumes: IO ports and Timers have been initialized.
void MotorRight(uint16_t leftDuty, uint16_t rightDuty){
    P3OUT |= LEFT_MOT_SLEEP;  //wake up motors
    P5OUT &=  ~LEFT_MOT_DIR;  //motors turn right
...
// Output: none
// Assumes: IO ports and Timers have been initialized.
void MotorLeft(uint16_t leftDuty, uint16_t rightDuty){
    P3OUT |= RIGHT_MOT_SLEEP;  //wake up motors
...
// Output: none
// Assumes: IO ports and Timers have been initialized.
void MotorBackward(uint16_t leftDuty, uint16_t rightDuty){
    P3OUT |=  RIGHT_MOT_SLEEP; //wake up right motor
    P3OUT |=  LEFT_MOT_SLEEP;  //wake up left motor

    P5OUT |= RIGHT_MOT_DIR;   //set right motor in back direction
    P5OUT |= LEFT_MOT_DIR;    //set left motor in back direction
