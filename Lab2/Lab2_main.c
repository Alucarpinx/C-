...
//Each time this function is called, one cycle of the PWM is output on the PWM pin
{
    ...
    P2OUT |=  LEFT_MOT_PWM;  //drive EN pins high for PWM
    // wait high time
    // The system clock is 48 Mhz,
    // for 25% duty cycle, high time is 2500 us
    // (2500us * 48MHz) = 120000
...
//Each time this function is called, one cycle of the PWM is output on the PWM pin
{
    ...
    P2OUT |= RIGHT_MOT_PWM;  //drive EN pins high for PWM
    // wait high time
    // The system clock is 48 Mhz,
    // for 25% duty cycle, high time is 2500 us
    // (2500us * 48MHz) = 120000
...
void main(void)
{
       ...
       enum motor_states {MoveForward, MoveBackward, TurnRight, TurnLeft, Sleep} state, prevState;

       state =  MoveForward;                  //start state
       prevState = !MoveForward;               //used to know when the state has changed
