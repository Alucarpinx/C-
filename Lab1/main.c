...
void main(){
    ...
    while(1) { if ((P1IN & BIT1) == 0) { //determine if S1 is pushed - remember it is active low
            P2OUT ^= 0b00000010; //(P2OUT |= BIT1); //toggle green on LED2
            while ((P1IN & BIT1) == 0); //wait here for pin to be released.
}
        if ((P1IN & BIT4) == 0) { //determine if S2 is pushed - remember it is active low
            P2OUT ^= 0b00000100; //(P2OUT |= BIT2); //toggle blue on LED2
            while ((P1IN & BIT4) == 0); //wait here for pin to be released.
